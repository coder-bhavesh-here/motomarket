            ...
            <div class="mt-6 px-8">
                ...
                <div class="flex mt-2 mb-6">
                    @foreach ($tour->addonGroups as $key => $group)
                        <a href="#{{strtolower(str_replace(' ','_', $group->name))}}" class="scoller text-lg text-green underline font-bold mr-2">{{ $group->name }}</a> {!! ($key + 1) < count($tour->addonGroups) ? "<span class='mr-2'>|</span>" : '' !!}
                    @endforeach
                </div>
